...
using NetZeroMQ.Benchmarks.Configs;
...
public class Program
{
    ...
    private static void TestPushPull()
    {
        ...
        try
        {
            using var ctx = new NetZeroMQ.Context();
            using var pull = new NetZeroMQ.Socket(ctx, NetZeroMQ.SocketType.Pull);
            using var push = new NetZeroMQ.Socket(ctx, NetZeroMQ.SocketType.Push);

            pull.SetOption(NetZeroMQ.SocketOption.Rcvtimeo, 3000);
            pull.SetOption(NetZeroMQ.SocketOption.Linger, 0);
            pull.Bind("tcp://127.0.0.1:15580");

            push.SetOption(NetZeroMQ.SocketOption.Linger, 0);
            ...
            using var msg = new NetZeroMQ.Message(16);
            pull.Recv(msg);
    ...
    private static void TestPubSub()
    {
        ...
        {
            using var ctx = new NetZeroMQ.Context();
            using var pub = new NetZeroMQ.Socket(ctx, NetZeroMQ.SocketType.Pub);
            using var sub = new NetZeroMQ.Socket(ctx, NetZeroMQ.SocketType.Sub);

            pub.SetOption(NetZeroMQ.SocketOption.Linger, 0);
            pub.Bind("tcp://127.0.0.1:15581");

            sub.SetOption(NetZeroMQ.SocketOption.Rcvtimeo, 3000);
            sub.SetOption(NetZeroMQ.SocketOption.Linger, 0);
            sub.Subscribe("");
    ...
    private static void TestReqRep()
    {
        ...
        {
            using var ctx = new NetZeroMQ.Context();
            using var rep = new NetZeroMQ.Socket(ctx, NetZeroMQ.SocketType.Rep);
            using var req = new NetZeroMQ.Socket(ctx, NetZeroMQ.SocketType.Req);

            rep.SetOption(NetZeroMQ.SocketOption.Rcvtimeo, 3000);
            rep.SetOption(NetZeroMQ.SocketOption.Linger, 0);
            rep.Bind("tcp://127.0.0.1:15582");

            req.SetOption(NetZeroMQ.SocketOption.Rcvtimeo, 3000);
            req.SetOption(NetZeroMQ.SocketOption.Linger, 0);
    ...
    private static void TestRouterRouter()
    {
        ...
        {
            using var ctx = new NetZeroMQ.Context();
            using var router1 = new NetZeroMQ.Socket(ctx, NetZeroMQ.SocketType.Router);
            using var router2 = new NetZeroMQ.Socket(ctx, NetZeroMQ.SocketType.Router);

            var router1Id = System.Text.Encoding.UTF8.GetBytes("r1");
            var router2Id = System.Text.Encoding.UTF8.GetBytes("r2");

            router1.SetOption(NetZeroMQ.SocketOption.Routing_Id, router1Id);
            router1.SetOption(NetZeroMQ.SocketOption.Rcvtimeo, 3000);
            router1.SetOption(NetZeroMQ.SocketOption.Linger, 0);
            router1.Bind("tcp://127.0.0.1:15583");

            router2.SetOption(NetZeroMQ.SocketOption.Routing_Id, router2Id);
            router2.SetOption(NetZeroMQ.SocketOption.Rcvtimeo, 3000);
            router2.SetOption(NetZeroMQ.SocketOption.Linger, 0);
            router2.Connect("tcp://127.0.0.1:15583");

            Thread.Sleep(100);

            // Router2 sends to Router1 first (handshake)
            router2.Send(router1Id, NetZeroMQ.SendFlags.SendMore);
            router2.Send(new byte[] { 1, 2, 3 });

            // Router1 receives (learns Router2's identity)
            using var msg = new NetZeroMQ.Message(64);
            router1.Recv(msg); // sender identity
            msg.Rebuild(64);
            router1.Recv(msg); // payload

            // Now Router1 can send to Router2
            router1.Send(router2Id, NetZeroMQ.SendFlags.SendMore);
